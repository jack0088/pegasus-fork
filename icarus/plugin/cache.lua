-- found on https://github.com/EvandroLG/pegasus.lua/commit/bc8c8f2fe1751f3e9cbddb93996e3fec97659f87
    ...
    if not file_info then
        ...
        if last_modified then
            if last_modified == os.date("!%a, %d %b %Y %T GMT", file_info.attrs.change) then
                ...
                response:contentType(response.headers["Content-Type"] or mimetypes.guess(filename))
...
function Cache:processData(data, stayOpen, request, response)
    if not response.filename or response.filename == "" then
        return data
    end
